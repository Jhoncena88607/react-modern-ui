...
type ActionType = "mint" | "burn" | "block" | "unblock" | "transfer";

export default function TokenActions() {
  const [selectedInvestors, setSelectedInvestors] = useState<string[]>([]);
  const [activeAction, setActiveAction] = useState<ActionType>("mint");
  const [mintValues, setMintValues] = useState<Record<string, number>>({});
  const [inputMode, setInputMode] = useState<"quantity" | "percentage">("quantity");

  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedInvestors(mockInvestors.map(i => i.id));
    } else {
  ...
  const handleMintValueChange = (investorId: string, value: string) => {
    const numValue = parseFloat(value) || 0;
    setMintValues({ ...mintValues, [investorId]: numValue });
  };
  ...
  const calculateNewBalance = (investor: Investor) => {
    const mintValue = mintValues[investor.id] || 0;
    if (activeAction === "mint") {
      return investor.balance + mintValue;
    }
  ...
  const calculateTotal = () => {
    return getSelectedInvestorsData().reduce((sum, inv) => {
      return sum + (mintValues[inv.id] || 0);
    }, 0);
  };
    ...
    <div className="flex-1 space-y-4 p-4 md:p-6 lg:p-8">
      ...
      {selectedInvestors.length > 0 && (
        <div className="bg-primary text-primary-foreground rounded-lg">
          ...
          {/* Action Tabs */}
          <div className="bg-background/10 border-t border-primary-foreground/20">
            ...
            <div className="p-4 bg-background text-foreground">
              <div className="flex flex-col lg:flex-row gap-4 mb-4">
                <div className="flex-1">
                  <p className="text-xs font-medium text-muted-foreground mb-2">
                    INVESTOR ({selectedInvestors.length})
                  </p>
                </div>
                <div className="flex-1">
                  ...
                  return (
                    <div key={investor.id} className="flex flex-col lg:flex-row gap-4 items-start lg:items-center">
                      <div className="flex-1 flex items-center gap-2">
                        <Avatar className="h-8 w-8" style={{ backgroundColor: getAvatarColor(investor.name) }}>
                      ...
                      <div className="flex-1">
                        <p className="text-sm">
                          {investor.balance.toLocaleString()} <span className="text-muted-foreground">USP</span>
                        </p>
                      ...
                      <div className="flex-1 flex gap-2">
                        ...
                        />
                        <div className="flex items-center justify-center text-xs text-muted-foreground px-2">
                          USP
                        </div>
                        <Input
                          type="number"
                      ...
                      <div className="flex-1">
                        <p className="text-sm">
                          {newBalance.toLocaleString()} <span className="text-muted-foreground">USP</span>
                        </p>
              ...
              <div className="mt-6 pt-4 border-t flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-lg font-semibold">{calculateTotal()} USP</p>
                </div>
                <div className="flex gap-2 w-full sm:w-auto">
                  <Button variant="ghost" onClick={() => setSelectedInvestors([])}>
                    Discard
                  </Button>
                ...
                <TableRow key={investor.id}>
                  <TableCell>
                    <Checkbox 
                      ...
                      onCheckedChange={(checked) => handleSelectInvestor(investor.id, checked as boolean)}
                    />
                  </TableCell>
                  <TableCell>
                    <Badge 
                      variant={investor.status === "Qualified" ? "default" : "outline"}
                      className={investor.status === "Qualified" ? "bg-success text-white" : ""}
                    >
                      {investor.status}
                    </Badge>
                  </TableCell>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1">
                      <span className="text-primary text-sm">{investor.wallet}</span>
